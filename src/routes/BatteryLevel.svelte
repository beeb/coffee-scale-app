...
<div class="flex text-sm items-center gap-2">
  {#if level >= 90}
    <BatteryFull class={classes} />
  {:else if level >= 70}
    <Battery75 class={classes} />
  {:else if level >= 45}
    <Battery50 class={classes} />
  {:else if level >= 20}
    <Battery25 class={classes} />
  {:else}
    <BatteryEmpty class={`${classes} text-error`} />
  {/if}
  {level}%
</div>
