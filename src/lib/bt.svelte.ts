...
export class Bluetooth {
	...
	private readBatteryLevel = async () => {
		...
		const service = await this.server.getPrimaryService(Number.parseInt('0x180F'))
	...
	connect = async () => {
		...
		try {
			// python firmware
			const service = await this.server?.getPrimaryService(Number.parseInt('0x1815'))
			this.weightCharacteristic = await service?.getCharacteristic(Number.parseInt('0x2A59'))
			this.newFirmware = false
		} catch {
			// rust firmware
			const service = await this.server?.getPrimaryService(Number.parseInt('0x181D'))
			this.weightCharacteristic = await service?.getCharacteristic(Number.parseInt('0x2A9D'))
